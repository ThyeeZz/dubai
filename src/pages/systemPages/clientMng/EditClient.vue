    ...
    >
      ...
      <el-form-item>
        <el-button type="primary" @click="submitForm('ruleForm');"
          >Submit</el-button
        >
        <el-button @click="goBack('ruleForm');">GoBack</el-button>
      </el-form-item>
    </el-form>
  </div>
</template>
...
import { updatePatient, refreshUser, getAllUser } from "@/api/api";
...
export default {
  ...
  created() {
    ...
    if (workTime.length) {
      this.worktime = workTime[0];
      this.workday = getWorkDay(workTime[0].week);
    }
    this.diy = (diy && true) || false;
    if (contact.length) {
      this.n = contact.length;
        ...
        if (valid) {
          updatePatient({
            ...
            .then(() => {
              ...
              this.$router.push({ path: "/index/system/client/search" });
            })
            .catch(err => {
              console.log(err);
            });
